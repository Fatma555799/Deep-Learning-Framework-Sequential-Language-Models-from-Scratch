...
```bash
Deep-Learning-Framework-Sequential-Language-Models-from-Scratch/
├── makemore_series/                       # Character-level Language Models (Andrej Karpathy course)
│   ├── micrograde_from_scratch.ipynb      # Micrograd implementation + exercises
│   ├── trigrad.py                         # MLP experiments & backpropagation essentials
│   ├── mlp2.py                            # Multi-Layer Perceptron basic architecture
│   ├── mlp_3.py                           # Deepening MLP structures
│   ├── makemore1.py                       # Basic Bigram model (counting + NN approach)
│   ├── build_makemore_mlp.py              # MLP-based character language model
│   ├── build_makemore_batchnorm.py        # Optimizing with Batch Normalization
│   ├── build_makemore_backprop_ninja.py   # Manual backpropagation ninja-level mastery
│   └── building_a_wavenet.py              # WaveNet-style hierarchical architecture
│
├── gpt/                                   # Generative Pre-trained Transformer from scratch
│   ├── gpt_bigram.py                      # GPT-style baseline Bigram language model
│   ├── gpt_v2.py                          # Scaled GPT model with Multi-Head Self-Attention
│   ├── input.txt                          # Training dataset (e.g., Tiny Shakespeare)
│   └── output.txt                         # Sample text generated by the model
│
├── gpt_calculator/                        # Algorithmic & Reasoning Experiments
│   └── calculator_gpt.py                  # Training GPT to perform math operations (+, -, *, /)
│
...
| **8** | `gpt_calculator/` | Shifting GPT from language patterns to algorithmic reasoning |
...
🧮 Special Feature: GPT Calculator Case Study
Instead of text, the model is trained on dynamically generated math problems (`+`, `-`, `*`, `/`). 
* **The Reverse Trick:** Problems are structured as `a + b = c_reversed` (e.g., `123+456=975`). This aligns the autoregressive generation (left-to-right) with the mathematical addition algorithm (right-to-left), enabling the model to manage carries flawlessly.
